...
function SSOHandler(
  ...
  this.submit = function () {
    ...
    var url =
      ...
      this.redirect_uri +
      "&state=state-aplsdfjoqjwdosjafñljaslñdfjaslñdfjlaksdfklasjdfklñasjfdlñasdflñk&code_verifier=" +
      this.code_verifier +
      "&code_challenge=" +
      this.code_challenge +
      "&idp=" +
  ...
  this.bwAuth = function (jwtSso) {
    debugger;
    nextiva_username =
      this.parseJwt(jwtSso)["com.nextiva.ident.voice.pid"] + "@nextiva.com";
    //this.callLoginValidation('adjflasjd','añlksdfjsaldfjsa',jwtSso);
    var authHeader = "Bearer " + jwtSso;
    var body =
      '{"communicatorName":"SFDCWebSock","httpContact":"http://192.168.1.128:9991/EventListner","applicationId":"Salesforce.com"}';
    $.ajax({
      method: "POST",
  ...
  this.callLoginValidation = function (username, pwd, oktaToken) {
    $(document).ready(function () {
      ...
      debugger;
      loginUserSso("añldfjaslñdfj", "añldsfjlasf", oktaToken, function (
        result,
        event
      ) {
        console.log(result);
        ...
        if (JSON.parse(result.status)) {
          ...
          saveCredentials(nextiva_username, nextiva_password, true, function (
            result,
            event
          ) {
            debugger;
